...
const MaybeDropdown = ({ children, countries, value, dropdown = false }) => {
  ...
  const options = React.useMemo(() => {
    return countries.map((c) => ({ text: c.name, value: c.id }));
  }, [countries]);
...
export const CountryFlagView = (props) => {
  ...
  React.useEffect(() => {
    if (countryCode) {
      ...
      import(
        ...
        `@eeacms/volto-datablocks/components/manage/Blocks/CountryFlag/data/svg/${code}.svg`
      ).then((module) => {
  ...
  const countries = React.useMemo(() => {
    return listingItems && listingItems.length > 0
      ? listingItems
      : siblings.filter((s) => s.name !== pageTitle);
